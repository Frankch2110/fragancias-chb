...
async function load(){
  const res=await fetch('catalog.json'); DATA=await res.json();
  buildBrandIndex(); render();
}
function peso(v){ // parse price_venta to number
...
function buildBrandIndex(){
  ...
  const bf=$('#brandFilter'); brands.forEach(b=>{
...
function filtered(){
  ...
  let rows=DATA.filter(r=>{
    const matchesQ= !q || (r.brand_general+' '+r.brand_base+' '+r.perfume+' '+r.size+' '+r.type).toLowerCase().includes(q);
  ...
  if(sort==='perfume') rows.sort((a,b)=>a.perfume.localeCompare(b.perfume));
